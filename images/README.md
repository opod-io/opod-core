...
| `opod-worker-sglang-nvidia` / `-amd` | `worker-sglang/` | lmsysorg/sglang (CUDA / ROCm) — the agent runs `sglang.launch_server` on model load, as for any engine core drives. One node per endpoint: core has no multi-node path for it, so the planner refuses a gang |
