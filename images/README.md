...
| `opod-worker-sglang-nvidia` / `-amd` | `worker-sglang/` | lmsysorg/sglang. The ROCm build is **Instinct only** (`…-mi30x` = MI200/MI300, `…-mi35x` = MI350; override with `SGLANG_AMD_BASE=`): there is no RDNA tag, so a Radeon card runs llama.cpp or vLLM instead — the agent runs `sglang.launch_server` on model load, as for any engine core drives. One node per endpoint: core has no multi-node path for it, so the planner refuses a gang |
