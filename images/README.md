...
| `opod-worker-vllm-intel` | `worker-vllm/` | intel/vllm (XPU: **Xe2/Xe3** — Arc B-series, Arc Pro B60 — and Max; **not Arc A-series**: on an A770 (2026-09-19) the engine started with `--dtype=half` and fp64 emulation, loaded the model, and died on the first request with upstream's `Only XE2/XE3 cutlass kernel is supported currently` — use `opod-worker-llamacpp-intel` there; no Xe2 card has served through this image yet) — vLLM per silicon is published by different people, and upstream `vllm/vllm-openai` is CUDA-only. Single-card serving until an Intel node proves more: we have run no Ray gang on the XPU backend. The base sets up oneAPI in its own ENTRYPOINT (`source /opt/intel/oneapi/setvars.sh --force`), which the worker's entrypoint replaces, so the build records that script in `/usr/local/share/opod/base-env` and `entrypoint.sh` sources it first (without it vLLM dies at `import torch`: `libccl.so.1` not found). Override with `VLLM_INTEL_BASE=` |
