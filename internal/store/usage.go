...
func (s *sqliteUsage) Record(ctx context.Context, u Usage) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO usage(ts, api_key_id, user_id, model, protocol,
		    prompt_tokens, completion_tokens, latency_ms, outcome, cost_usd, node_id, ttft_ms)
		 VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
		u.TS.Unix(), u.APIKeyID, u.UserID, u.Model, u.Protocol,
		u.PromptTokens, u.CompletionTokens, u.LatencyMS, u.Outcome, u.CostUSD, u.NodeID, u.TTFTMS) // CostUSD: see the type's note — always 0, deprecated (ADR-003)
	return err
}
...
func scanUsage(rows *sql.Rows) ([]Usage, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &ts, &u.APIKeyID, &u.UserID, &u.Model, &u.Protocol,
			&u.PromptTokens, &u.CompletionTokens, &u.LatencyMS, &u.Outcome, &u.CostUSD, &u.NodeID, &u.TTFTMS); err != nil {
			return nil, err
		}
		u.TS = time.Unix(ts, 0)
		out = append(out, u)
