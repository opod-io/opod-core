...
"Live" is decided in one place, from the node row, and never stored: `store.Node.TakesNewWork(maxAge, now)` (`internal/store/nodelive.go`; `WhyNoNewWork` is the same rule with the reason, `LiveState` the state to show). A node takes new work when it is not drained, its state is a serving one, it heartbeated within `router.heartbeat_max_age_seconds`, and its engine has not been silent for longer than that (below). Everything that decides or reports on workers asks it:
...
**What `/readyz` answers, and why it is almost never 503.** A healthy local engine is `ready`; no local engine but an alive worker holding a routable placement is `router-only`; a gang whose coordinator is ready and whose every part is on an alive node is `shard-coordinator`; workers whose engines sleep are `sleeping-workers`. Only then does the mounted plan's autoscale floor come into it, and it splits in two: with no worker up at all the endpoint is `sleeping` — parked on purpose, and the pod must stay Ready so its Deployment converges — while with workers registered and nothing able to serve it is **`waking`**. Both are 200. Failing the probe is what would take the leader out of its Service and turn the honest `503` + `Retry-After` into a connection error, and that 503 is the request that brings the workers back. The distinction is not cosmetic: the floor-0 branch used to be asked first and answer `sleeping` for all of it, so a gang that was serving reported itself parked, and a gang whose parts were up but could never form reported a healthy parked endpoint to Kubernetes, the console and the operator alike (both seen on the design-partner cell, 2026-09-20).

**A worker says goodbye.** A worker sends one final heartbeat on its way out (`agent.Goodbye`, feature
`worker_goodbye`), declaring its engine `stopped` with `detail: worker shutting down`. It exists because of a
window every park and every rolling update had: the pod gets SIGTERM, the engine — or an RPC part of a gang —
dies with it, and the worker goes on heartbeating through the grace period with its last good report. For
those seconds the leader believed the capacity was there, picked the dead part, and answered the caller **502**
— the one error class §"Nothing left" says must never happen. The worker is the only party that can know: the
leader sees a heartbeat and Kubernetes sees a container that is still Running. The leader acts on that ONE
state (`Server.engineGoneWhy`): a fresh `stopped` takes the worker out of the router's pick, out of
`modelServable`, out of `routableNodes` — so a gang's servability follows — and out of `/loadz`'s worker
count, while `engines_unhealthy` still counts it, because the card it holds is still held. `crash-looping`
does **not** take a worker out: the process may be up again by the time the next request lands, and flapping
an endpoint between serving and 503 on a report is worse than routing one request at a worker that then
errors. Silence remains the heartbeat rule's alone. The goodbye is best-effort and bounded at 3 s — a shutdown
must never wait on a leader that is itself restarting, and a lost goodbye costs exactly what we had before,
the heartbeat bound (ADR-065).

**Between a worker's last heartbeat and the day "live" notices.** A worker that has just gone away (its pod was moved, its machine died) is still live by the rule above until its heartbeat ages out, so a request can be picked for it. It cannot connect — `engines.ErrUnreachable`, a transport failure before any byte of an answer, known within `openaicompat.ConnectTimeout` (3 s: a removed pod's address hangs rather than refuses, and Go's default would wait 30 s) — and the walk then picks the **same model again with that node set aside for this request only** (`internal/router/nextworker.go`; chat and embeddings). Each worker can be set aside once, so the walk ends; when none is left the first worker's error is what the caller hears, and the fallback chain goes on as before. A request is assigned to a revision group **once**: the re-pick stays inside that group while it still has a worker, so asking the next worker never re-rolls a traffic split (rolled again on every retry, a 20 % canary measured 36 % while a removed worker lingered in the old group). The cooldown counters are what keep *later* requests away. An engine's own answer, a refusal included, is never replayed on another worker.

**A heartbeat without an engine report.** A worker asks its engine what it holds on every heartbeat (every 5 s, waiting 2 s). When the engine does not answer in time (or answers with an error) the heartbeat still goes out, with `loaded_models: null` — *no report*, which is not the same as `[]`, *nothing is loaded*. An engine that is **not running** is the second case, not the first: a refused connection means nothing listens, so nothing is loaded, and the worker says `[]`. That distinction keeps a worker whose engine is launched *by* a load (vLLM, SGLang, llama.cpp) and every part of a llama.cpp RPC gang (an rpc-server, never an engine) in rotation — reported as "no report" they went `engine-silent` 30 s after joining, and a gang's leader never became ready. The leader treats the two differently (`HeartbeatNode`): a report-less heartbeat advances the node's liveness and **changes no placement row** — statuses, the leader's `draining` and `released` marks and cold flags stay exactly as they are — while `[]` clears the node's rows as it always did. (It used to be one case: a null list replaced the rows with nothing, which deleted the marks, and the next real report recreated the rows `ready` — a single slow engine tick put a draining node's placement, or the source of a model move, back in rotation.) So that a worker whose engine **never** answers again does not keep routable rows for ever — heartbeat age cannot catch it, the worker *is* heartbeating — the node row records since when its heartbeats have carried no report (`nodes.engine_silent_since`, stamped by the first one, cleared by the first real report; a row fact rather than a leader-side counter, because the rule is derived from the row and is also asked by readers that have only the row — the router, the gateway's model list, the CLI's shard-count picker), and the one rule above takes the node out of rotation once that is longer than the same bound a silent worker gets (`router.heartbeat_max_age_seconds`, 60 s when that check is off — this rule has no off switch): a dozen consecutive unanswered probes, not a slow tick. Nothing is deleted or rewritten: the node's rows and marks are as they were, and the first real report puts it back with no reconciliation step. What an operator sees: `opod node ls` and `GET /admin/v1/nodes` show the state `engine-silent`; refusals read "its engine has not answered it for 1m35s"; a model whose every holder is in that state answers `503` + `Retry-After` with "N worker(s) whose engine stopped answering"; and the journal carries `node.engine_silent` once when the node leaves rotation and `node.engine_reporting` once when a report brings it back — never one event per slow tick.
...
CREATE TABLE model_placements (
    ...
    cold       INTEGER NOT NULL DEFAULT 0,  -- 1 = the worker answers for it, it is not in memory now
    PRIMARY KEY (node_id, model_id)
);
CREATE INDEX idx_placements_model ON model_placements(model_id);
```

Worker heartbeats carry `loaded_models` — **what the worker answers a request for**. For vLLM, SGLang and llama.cpp that is what is in memory. For an engine that keeps installed models and loads one on its first request (Ollama) it is the installed list, and it has to stay that: the router routes by these rows, and a row that vanished when Ollama's idle timer evicted the model would stop traffic to a worker that can answer. Such an engine therefore ALSO sends `resident_models` (the engine's `/api/ps`; sent even when empty, omitted by an engine that cannot tell the two apart), and the leader records the difference on the row as `cold`: routable like any other row, holding no memory. `scheduler.WorkerMemoryFacts` — the shard-count picker's and the model move's memory facts — skips cold rows; it used to count every installed Ollama model as resident. The leader calls `PlacementStore.ReplaceForNode(nodeID, …)` to reconcile atomically every 5s. Local placements (`node_id="local"`) are populated by `cmd_model.go` on add and by `cmd_up.go` on startup (it lists the leader's local engine).
...
**Journal.** Every step is an event on the leader's stream, subject = the model: `model.move_started`, `model.move_loaded`, `model.move_flipped`, `model.move_drained` (`inflight_left`, `timed_out`), `model.move_finished`, or `model.move_aborted` (`reason`, `state` — what serves now). The route is synchronous and returns the same steps.

**A leader that restarts mid-move abandons it, safely.** Nothing about a running move is persisted except the one mark it may have set, and a leader start lifts every `draining` mark (`liftStaleDrains`). Before the flip the source was never touched. After the flip and before the unload, the source — still holding the model — is simply serving again. In both cases the copy the target loaded stays: it is a second replica, which the router load-balances to like any other; nothing is wrong, the model is resident twice, and the operator may run the move again (the target then "already serves") or unload either side. There is no store-only path for the same reason: it is the leader's router that flips and the leader's counters that say the source is idle.
...
**What it is not.** No KV-cache transfer. Every conversation that was being served by the source loses its prefix cache: its next turn is recomputed from the prompt on the target (slower first token, same answer). The router's prefix-affinity and sticky pins that point at the source need no clearing: a pin only reorders the workers that can take the request, and after step 4 the source is not among them. It is not a way to move a sharded model (a gang is rebuilt with `opod shard create`), and it does not carry LoRA adapters: a source that serves adapters of the model is refused — drop them (`DELETE /admin/v1/adapters/{name}`), move the base, add them again.
...
| Ollama | pull + warm load, synchronous; other models stay resident | the heartbeat lists *installed* models (the row appears once pulled) and, beside them, the *resident* ones: the row stops being `cold` once the weights are in memory. The load call is synchronous and its success says the same | an engine unload frees the memory, but the model stays installed, so the heartbeat keeps listing it and the row stays — as `cold` once the worker reports it out of memory. The move then marks the row `released` (mechanism 4): not routed to, across heartbeats and leader restarts |

**The four mechanisms it stands on** — each small and additive, each with its own feature key:

1. **A worker-side unload — built (feature `worker_unload`).** `POST /v1/model/unload` on the worker, HMAC-signed like `/v1/model/load` and carrying the same source fields minus `file` and `pin` (`{id, ollama_name, repo, path}`), so the worker resolves the engine-native name exactly as the load did. What it does depends on whose process the engine is: an engine the worker launched for the model (`vllm-serve`, `sglang-serve`, `llama-server` — the worker records each launch) is stopped through the supervisor, and the alias, the LoRA launch record and any start-up adapter loader still waiting go with it; an engine that outlives its models is asked through the driver's `Unload` (Ollama; the weights and the alias stay). Answers: `200 {"status":"unloaded","model"}`; `200 {"status":"noop","reason"}` when the model is not resident (idempotent — a second call, or a worker whose one engine serves something else); `409` plain text when a shard part of the model runs there (naming the process ids) or adapters of that base are still held (naming them) — both have their own routes; `501 {"status":"unsupported","engine","reason"}`, the shape `/v1/model/sleep` uses, when the driver cannot unload and the worker did not start the engine; `502` when the engine failed. The leader's half is `scheduler.UnloadFromNode` (a typed `WorkerRefusal` keeps 409/501/502 apart); it writes no placement row — the model leaves the worker's next heartbeat and the row, with any `draining` mark, goes then. The leader's `POST /admin/v1/models/{id}/unload` still acts on the leader's own engine only.
2. **A flip that survives a heartbeat — built (feature `placement_drain`).** A placement whose status is `draining` is invisible to `Placements().GetByModel`, which is how the local lifecycle drains before an eviction; on a worker the mark used to last one heartbeat, because every heartbeat rewrote the node's rows as `ready`. `ReplaceForNode` now carries the mark: inside its one transaction a row that was `draining` stays `draining` while the model is still in the worker's report, and goes with the row when the model leaves it. The heartbeat body is unchanged — a worker reports residency, never routability. The mark is set and lifted with `Placements().SetStatus` (the move's step 4 sets it; no route sets it on its own), and since it now lives in the store rather than in one heartbeat interval, a leader start lifts every mark a previous process left (`liftStaleDrains`) — a restart abandons the move and the source, never unloaded, is simply serving again.
